...
(function (spine) {
	var SkeletonJson = (function () {
		...
		SkeletonJson.prototype.readAnimation = function (map, name, skeletonData) {
					...
					for (var timelineName in slotMap) {
						...
						if (timelineName == "color") {
							...
							for (var i = 0; i < timelineMap.length; i++) {
								...
								var tempTime = this.getValue(valueMap, "time", 0);
								timeline.setFrame(frameIndex, tempTime, color.r, color.g, color.b, color.a);
							...
							for (var i = 0; i < timelineMap.length; i++) {
								var valueMap = timelineMap[i];
								var tempTime = this.getValue(valueMap, "time", 0);
								timeline.setFrame(frameIndex++, tempTime, valueMap.name);
							}
							timelines.push(timeline);
			...
			if (isNaN(duration)) {
				throw new Error("Error while parsing slots animation, duration is NaN");
			}
			if (map.bones) {
					...
					for (var timelineName in boneMap) {
						...
						if (timelineName === "rotate") {
							...
							for (var i = 0; i < timelineMap.length; i++) {
								var valueMap = timelineMap[i];
								var tempTime = this.getValue(valueMap, "time", 0);
								timeline.setFrame(frameIndex, tempTime, valueMap.angle);
								this.readCurve(valueMap, timeline, frameIndex);
								frameIndex++;
							}
							timelines.push(timeline);
							...
							for (var i = 0; i < timelineMap.length; i++) {
								...
								var tempTime = this.getValue(valueMap, "time", 0);
								timeline.setFrame(frameIndex, tempTime, x * timelineScale, y * timelineScale);
								this.readCurve(valueMap, timeline, frameIndex);
								frameIndex++;
							}
							timelines.push(timeline);
						...
						if (isNaN(duration)) {
							throw new Error("Error while parsing bones animation, duration is NaN ," + boneName + ",timelineName: " + timelineName);
						}
					}
				}
			}
			...
			if (isNaN(duration)) {
				throw new Error("Error while parsing before draw animation, duration is NaN");
			}
			...
			if (isNaN(duration)) {
				throw new Error("Error while parsing final animation, duration is NaN");
			}
